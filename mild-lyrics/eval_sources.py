...
instead -- the ones people hand-timed and uploaded, held on disk here from
when they were fetched. There are more of those on a machine that has been
listening than there are files in ./lyrics, and they are the yardstick the
songs were judged against by ear.
...
def spicy(tid: str):
    ...
    Off the disk, never over the wire: this is a measuring run over hundreds
    of tracks, and a source it is not measuring is not worth a request each.
    """
    try:
        return LS.spicy_held(tid) or None
    except Exception:                                    # noqa: BLE001
        return None
...
def references(where: str) -> dict:
    ...
    Walked rather than listed: the lyrics folder has rooms in it now -- what
    the player saved and what was timed in the editor, kept apart because they
    are not the same kind of file (see saves.py) -- and the ones timed by hand
    are the whole point of this. A reference is judged by what is in it, not
    by which room it is in, so both are read and only word-timed documents
    ...
    if not root.is_dir():
        return out
    for path in sorted(root.rglob("*.ttml")):
...
def donors_report(picked, refs, use_spicy: bool, fold: bool,
                  limit: int) -> dict:
...
def main() -> int:
    ...
    ap.add_argument("--no-spicy", action="store_true",
                    help="do not read Spicy Lyrics' own copy at all")
    ...
    if args.donors:
        got = donors_report(picked, refs, args.against == "spicy", fold,
                            args.songs)
        if not got["rows"]:
    ...
    for tid, v in picked:
        ...
        if not meta["title"]:
            continue
        body = None if args.no_spicy else spicy(tid)
        key = f"{LS._norm(meta['title'])}\x00{LS._norm(meta['artist'])}"
