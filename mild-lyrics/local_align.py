...
def local_decoys(exclude: str = "", n: int = 6) -> list[list[str]]:
    ...
    already, in `lyrics/` and the rooms inside it, is every other song this
    project has a TTML for: plenty for a floor, and free, because nothing here
    is fetched or timed -- only read.
    ...
    """
    try:
        files = [f for f in LYRICS_DIR.rglob("*.ttml")
