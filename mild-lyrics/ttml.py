...
def index(cdp, refresh: bool = False) -> dict:
    """Every song a lyric is held for, by track id, kept on disk.
    ...
    So it is asked once and the answers are kept: the list only grows when
    something new is fetched, and --refresh picks those up.

    The ids come from this machine's own lyric cache (LS.spicy_ids). Spicy
    Lyrics' API answers about one track and does not enumerate, so what can be
    listed is what has been asked for here -- every song the window has drawn
    is in it.
    """
    got = {}
    ...
    try:
        with LA.fetched(f"{meta['artist']} {meta['title']}",
            ...
            got = LA.align(audio, doc, want="gpu", spare=args.spare,
                           ...
                           decoys=decoys(), onsets=not args.no_onsets,
                           log=log)
