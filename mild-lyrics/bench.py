...
def cached(tid: str):
    """Spicy Lyrics' own document for a track, parsed, or None."""
...
def every_referenced(cdp, want: int) -> list[dict]:
    """Songs held here with WORD-SYNCED timings, in a stable order.

    Ordered by a hash of the name, not by when the song was fetched: the set
    of songs held grows every day, and a set that re-draws itself between runs
